...
const Input = ({
  ...
  return (
    <div className="relative flex flex-col space-between bg-mob-shortenImg md:bg-desk-shortenImg bg-DarkViolet bg-[left_6rem_bottom_2rem] md:bg-center bg-bg-once bg-no-repeat py-4 xxs:py-6 md:py-12 px-4 xxs:px-8 md:px-12 lg:px-16 rounded-xl w-full text-sm xxs:text-base md:text-lg">
      <form
      ...
      >
        <div className="flex flex-grow flex-col gap-y-1">
          ...
          {inputStatus && fetchError === "" ? (
            <i className="md:absolute md:bottom-4 md:left-16 text-Red text-xs xxs:text-sm md:text-lg">
              Please add a link
            </i>
          ) : null}
          {fetchError !== "" ? (
            <i className="md:absolute md:bottom-4 md:left-16 text-Red text-sm xxs:text-base md:text-lg">
              {fetchError}
            </i>
          ) : null}
          <label
